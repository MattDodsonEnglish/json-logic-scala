...
class TestSerializerConf extends AnyFlatSpec with Matchers {
  ...
  "createConf className not defined in props file" should "throw an exception" in {
    val thrown = the[ConfigurationException] thrownBy {SerializerConf.createConf("META-INF/services/json-logic-scala/tests/serializer/exceptions/classname-not-defined-exception-class/")}
  ...
  "createConf codename not defined in props file" should "throw an exception" in {
    val thrown = the[ConfigurationException] thrownBy {SerializerConf.createConf("META-INF/services/json-logic-scala/tests/serializer/exceptions/codename-not-defined-exception-class/")}
  ...
  "createConf wrong constructor argument names in props file" should "throw an exception" in {
    val thrown = the[ConfigurationException] thrownBy {SerializerConf.createConf("META-INF/services/json-logic-scala/tests/serializer/exceptions/wrong-constructor-argument-names-definition-exception-class/")}
    val expectedMessage = "Field error, check that no field in 'com.github.celadari.jsonlogicscala.serialize.impl.MarshallerStringImpl2' is missing in 'string' property file.\nCheck that no property in 'string' file is not undefined in 'com.github.celadari.jsonlogicscala.serialize.impl.MarshallerStringImpl2' class.\nCheck if 'com.github.celadari.jsonlogicscala.serialize.impl.MarshallerStringImpl2' class constructor requires arguments or if argument names defined in 'string' property file are correct"
    thrown.getMessage shouldBe expectedMessage
  }

  "createConf wrong property type in props file" should "throw an exception" in {
    val thrown = the[ConfigurationException] thrownBy {SerializerConf.createConf("META-INF/services/json-logic-scala/tests/serializer/exceptions/wrong-property-type-exception-class/")}
