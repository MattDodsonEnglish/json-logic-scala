...
class TestOperatorSome extends TestNumeric with TestArray {

  "Operator Some f(el: Boolean => el)" should "return value" in {
    val tree = new ComposeLogic("some", Array(
  ...
  "Operator Some f(el: String => el.substring(string0))" should "return value" in {
  ...
  "Operator Some more than 2 input conditions" should "throw an exception" in {
    val string0 = "You love New York"
    val tree = new ComposeLogic("some", Array(
      ValueLogic(Some(arrString), Some(arrStringType)),
      new ComposeLogic("in", Array(
  ...
  "Operator Some less than 2 input conditions" should "throw an exception" in {
    val tree = new ComposeLogic("some", Array(
      ValueLogic(Some(arrString), Some(arrStringType)),
    ))
  ...
  "Operator Some f(el: Float => el**2 + el + 3 == 4)" should "return value" in {
    val tree = new ComposeLogic("some", Array(
